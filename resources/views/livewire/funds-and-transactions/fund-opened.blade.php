<li class="funds_section_lists_container_in_process_list_item flex">

    <div class="funds_section_lists_container_in_process_list_item_name_and_amount_container">

        <p class="funds_section_lists_container_in_process_list_item_amount_text hel_reg_it">
            {{number_format($fund->amount/100, 2, ',', ' ')}}&euro;</p>

        <a class="funds_section_lists_container_in_process_list_item_link hel_reg" wire:navigate
           href="{{route('funds_and_transactions.show', $fund)}}"
           title="{{__('texts.see_details')}} {{$fund->name}}">{{$fund->name}} &ndash; {{$fund->pourcentage}}
            %</a>

    </div>

    @if($fund->id !== 1 && $fund->id !== 2)
        <button class="modal_btn delete" title="{{__('texts.make_this_fund_enclosed')}}" wire:click="dispatchTo('messages.fund-enclosed-confirmation-message', 'openModal', [{{$fund->id}}])">

            <svg class="modal_btn_icon" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7 21C6.45 21 5.97917 20.8042 5.5875 20.4125C5.19583 20.0208 5 19.55 5 19V6H4V4H9V3H15V4H20V6H19V19C19 19.55 18.8042 20.0208 18.4125 20.4125C18.0208 20.8042 17.55 21 17 21H7ZM17 6H7V19H17V6ZM9 17H11V8H9V17ZM13 17H15V8H13V17Z" fill="#1D1B20"/>
            </svg>

        </button>
    @endif

</li>
